...
## 当前状态总览（2026-05-16 snapshot）

> 这一段是面对**当前进度**的速查，权威来源是本文件末尾的 "Verification log"。如果一段陈述与 Verification log 冲突，以 log 为准。

**已经全部关闭的工作**：

- A1 / A2 / A3 / A4：操作和 brief 文档对齐都做完了。
- B1 / B2 / B3 / B4：B1-B3 全部完成（B3 走 `plan/zhihurec-v1-cold-start-mixing/`，已 verified）；B4 状态特征 audit 完成，主动按 brief §14 边界**不实现**多数项。
- C1 / C3：数据分析报告完成（见 `plan/zhihurec-v1-data-analysis-report/`，`docs/data_analysis_report.md` §1-7 + 12 张图）；简历 bullet 完成（`docs/resume_bullet.md`）。
- D1 / D2：两处文档卫生修完。
- **E1-E7（plan/zhihurec-v1-quality-upgrade/，2026-05-16 完成）**：质量基础设施补齐——版本钉版本、`.env.example`、`pyproject.toml`、ruff/mypy/pytest 三道护栏、根 README。不在原 A/B/C/D 分类里，但已是 V1 收尾的重要一环。
- **离线 ranking 指标首条基线（2026-05-16）**：新增 `backend/app/evaluate.py` + `scripts/eval_offline_metrics.py` + `tests/test_evaluate.py` + `docs/v1_metrics.md` 的 "Item-Ranking Recall@K and NDCG@K" 节，并由 `plan/zhihurec-v1-offline-eval/` 收成正式 V1 deliverable。首基线 Recall@10 = 0.0000 / NDCG@10 = 0.0000 / candidate_recall@50 = 0.1579（19 test clicks）。结论：系统能 topic-aligned（Carryover@10=1.0）但还不能预测具体 `answer_id`，下一步是 retrieval 深度而不是排序权重。

**仍然 open 的一件事**（其它都已收完）：

1. **C2 HCI 报告 §4 + §6**：`docs/hci_report.md` 的 §4（4 段截图叙事）和 §6（评估方法 + N=3-5 走查访谈摘录）仍是骨架，文件中明确写着 "> 待补"。要做需要 `scripts/init_local.ps1` 拉起本地栈，加真人走查。
...
### A3. brief §14 与 docker compose 的矛盾 ✅ 完成 (2026-05-01)
...
### A4. brief §18 当前仓库基线已严重过期 ✅ 完成 (2026-05-01)
...
### B1. Replay 三类场景覆盖确认 ✅ 完成 (2026-05-01)
...
### B2. Search Carryover Gain@K 指标实现 ✅ 完成 (2026-05-01, Gain@10 = 0.1000)
...
### B3. behavior_score → alpha gating 是否真实参与排序 ✅ 完成 (2026-05-01, see `plan/zhihurec-v1-cold-start-mixing/`)
...
### B4. Feed-to-search 廉价状态特征 🔍 已审计 (2026-05-01)，按 brief §14 边界主动不实现
...
### C1. 可视化数据分析报告 ✅ 完成 (2026-05-02, see `plan/zhihurec-v1-data-analysis-report/`)
...
### C2. HCI 报告 🟡 部分完成 (§1-3 / §5 / §7 / §8 已写；§4 截图叙事 + §6 评估方法待补)
...
### D1. runtime-closed-loop README "Current status" 段已过期 ✅ 完成 (2026-05-01)
...
### D2. v1_local_runbook.md §3 没提 docker ✅ 完成 (2026-05-01)
...
1. D:\Github\zhihurec\plan\zhihurec-v1-gap-checklist\README.md（这份文件，本项目所有"还没做的事"的总入口）
   - 先看顶部"当前状态总览（2026-05-16 snapshot）"段，3 分钟内掌握全局进度
   - 再跳到末尾"Verification log"看每一条历史日志条目；这是 ground truth
...
- gap-checklist 的 A1-A4 / B1-B3 / C1 / C3 / D1 / D2 全部 ✅；B4 已审计后主动按 brief §14 边界不实现；**仅 C2（HCI 报告）§4 截图叙事 + §6 评估方法走查仍待补**。
- B2 基线：baseline 0.9000 / replay 1.0000 / Gain@10 = 0.1000（121 事件，三类齐）。
- B3 实现：feed ranking 已用 `compute_alpha(behavior_score)` 在 personalized / default topic 上线性混合；`/feed?debug=true` 暴露 `cold_start_mix`。
- E plan（质量基础设施）E1-E7 完成：`requirements.txt` 钉版本、`.env.example`、`pyproject.toml`（ruff/mypy/pytest 配置）、ruff 0 violations、mypy 0 issues across 31 files、默认 pytest 21 passed / -m mysql 4 passed、根 README。
- 离线 ranking 首条基线已由 `plan/zhihurec-v1-offline-eval/` 收口：Recall@10=0.0000 / NDCG@10=0.0000 / candidate_recall@50=0.1579（19 test clicks）。这不是 V2 模型升级，只是 V1 的 honest baseline。
- 本地分支 ahead origin/main，未 push；push 仍由用户单独决定。
- 容器和 backend 全停。

当前没有 "活跃 plan"——offline-eval 已 formalized，上一轮 quality-upgrade 也已收尾。下一步由用户从下面两个方向挑：
...
- 2026-05-02 — C2-step2 — `docs/hci_report.md` §5 / §7 / §8 落地：§5 把可见性拆成 item / request / profile 三层（含 5.5 七步走查脚本），全部锚到 `backend/app/schemas/feed.py:7-13,23,25,40-51` 与 `frontend/app.js:76-77,135-145,236`；§7 把 brief §14 / §17 边界、C1 §5 demo 子集、B4 状态特征 audit、长尾压抑、replay 样本量都列成显式局限；§8 把 §1-3 的 12 条目标/画像 1:1 对到 `docs/v1_metrics.md`、`docs/data_analysis_report.md`、`docs/v1_api_contract.md` 的具体数字与字段。HCI 报告剩 §4 截图叙事和 §6 评估方法，需要起容器 + 走查时再补。
- 2026-05-16 — mysql repo refactor — 在 E plan 之前先做了两个非 plan 内的关键 cleanup：commit `49edee6 fix(repo): close three silent-failure paths in mysql repository`（堵了 3 处 silent SQL exception）和 commit `f03059c refactor(repo): decompose mysql.py into focused DAO modules`（把巨型 mysql.py 拆成职责清晰的 DAO 模块）。这两步是 E plan ruff/mypy 通过的前置基础，没有自己的 plan dir，靠 commit message 自洽。
- 2026-05-16 — E plan complete — `plan/zhihurec-v1-quality-upgrade/` 7 步全部完成：散文件清理 + 依赖钉版本 + `.env.example` + `pyproject.toml` + ruff/mypy 全绿 + pytest 默认 21 / mysql 4 + 根 README。详见该 plan 的 Verification log。
- 2026-05-16 — offline-eval — 新增 `backend/app/evaluate.py`（`time_split`/`recall_at_k`/`ndcg_at_k`，19 个 pytest 用例全绿）+ `scripts/eval_offline_metrics.py` 驱动 + `docs/v1_metrics.md` 新章 "Item-Ranking Recall@K and NDCG@K"。首条基线 `--k 10 --train-ratio 0.8` 在真实 docker MySQL + uvicorn 上跑通：97 train 事件全部 200，19 test clicks 中 **Recall@10 = 0.0000 / NDCG@10 = 0.0000 / candidate_recall@50 = 0.1579**。诚实首数：系统能 topic-aligned（Carryover@10=1.0）但还不能预测具体 `answer_id`，下一步该升 retrieval 深度而非排序权重。
- 2026-05-17 — offline-eval plan — `plan/zhihurec-v1-offline-eval/` 已建立，作为离线 item-ranking baseline 的正式 V1 deliverable 入口；本 checklist 顶部状态和 resume prompt 已改为指向该 plan；`tests/test_evaluate.py` 19 passed，默认 pytest 40 passed / 4 deselected，真实 docker MySQL + uvicorn baseline 复跑一致（Recall@10=0.0000 / NDCG@10=0.0000 / candidate_recall@50=0.1579）。
